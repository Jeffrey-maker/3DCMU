...
The interface supports rotation, zoom, floor filtering, optional room-number overlays, animated routes, stairs, elevators, and escalators. The `Room numbers` checkbox shows or hides labels on the selected floor without hiding its room geometry. For standard trips, the planner compares the connector alternatives that are reachable through the mapped floor walkspace and recommends the lowest-cost route. Accessible trips use elevators only.
