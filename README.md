...
Walls and validated doors share the same architectural 3D layer and vertical dimensions: walls are dark-gray rectangular prisms and doors are green rectangular prisms. The interface supports rotation, zoom, floor filtering, optional room-number overlays, animated routes, stairs, and elevators. The `Room numbers` checkbox shows or hides labels on the selected floor without changing walls or floor areas. For standard trips, the planner compares reachable stairs and elevators and recommends the lowest-cost route. Accessible trips use elevators only.
