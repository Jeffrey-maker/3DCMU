...
The 3D map loads all 1,262 labeled spaces extracted from the 16 supplied Scott Hall and Wean Hall plans. Rooms are represented by their plan footprint and optional room-number label rather than artificial cubes. Long horizontal and vertical wall strokes are detected from each rendered 2D plan, merged to remove line thickness, and extruded into instanced 3D wall segments.
...
Walls and validated doors share the same architectural 3D layer and vertical dimensions: walls are dark-gray rectangular prisms and doors are green rectangular prisms. Door prisms are rotated along their wall using the validated passage attachment normal, with nearest-wall orientation as a fallback. The phone-sized interface supports rotation, zoom, floor filtering, optional room-number overlays, a thick bright-red 3D route raised above the walls, a draggable route-directions sheet, numbered 3D instruction nodes with step-to-map camera focus, two modeled staircase stacks per building, and elevators. The `Room numbers` checkbox shows or hides labels on the selected floor without changing walls or floor areas. For standard trips, the planner compares every reachable staircase and elevator candidate and recommends the lowest-cost route. Accessible trips use elevators only.
